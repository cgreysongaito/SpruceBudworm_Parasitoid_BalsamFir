...
interaction<-interactionimport%>%
  ...
  filter(!SpecID.high %in% c("pA3", "p40","p05", "p07","p04"))%>% #remove all unresolved taxa
...
propSBWALTbyspeciesdata<-interaction%>%
  ...
  mutate(SpecID.high=as.factor(SpecID.high)) #This section produces a dataset of the Proportion of emergences from spruce budworm for each parasitoid taxon. also gives the proportion standard error and error bars for the standard error.
...
#Community-level resource tracking -----
...
ratioSBWparacommunity<-interaction%>%
  ...
  mutate(ratioSBWEmerge=TotSBWEmerge/TotALTEmerge,logratioSBWEmerge=log10(ratioSBWEmerge)) #produces a data set of the ratio (and log 10 ratio) of the total number of parasitoid emergences (all taxa combined) from spruce budworm and other caterpillars sampled on balsam fir.

communitydietswitch<-full_join(ratioSBWparacommunity,ratioSBWbypeakplot,by=c("Peak","Plot"))%>%
  filter(TotSBW!=0,TotALT!=0,TotSBWEmerge!=0,TotALTEmerge!=0)%>% #Removed four data points (from a total of 21 data points).
  ungroup()%>%
...
#AIC lowest for model without autocorr (and ACF does not show improvement). Changes too small so not including corAR1.
...
# Interaction is significant (p-value = 0.0039, delta AIC = 7). Therefore, keep all fixed effects in the model including the interaction.
...
#AIC lowest for model with varIdent but without autocorr (But ACF shows marked improvement)
...
anova(SBWALTnumvarIdentautocorrmlfull,SBWALTnumvarIdentautocorrdrop3interactionml) #three way interaction not significant at level considered significant in Zuur
...
SBWALTnumfinaldropint<-update(SBWALTnumvarIdentautocorrdropPlot,.~.-dummyPeak:SBWALT)
anova(SBWALTnumvarIdentautocorrdropPlot,SBWALTnumfinaldropint) #gives L and p values for interaction fixed effect
...
#From AIC values varident is best method to deal with heterogeneity (AIC is 2675.4430 versus 358.7936 for without corr structure and 265.9747 for varPower)
...
#7. Refit the model found in step 6. with REML estimation
SBWALTEMfinalmodel<-gls(AvNEM~dummyPeak+SBWALT+Plot+dummyPeak:SBWALT+SBWALT:Plot,weights=varIdent(form=~1|SBWALT),data=SBWALTEM, method="REML") 
...
SBWALTEMvaridentremlGLS<-Gls(AvNEM~dummyPeak+SBWALT+Plot+dummyPeak:SBWALT+SBWALT:Plot,weights=varIdent(form=~1|SBWALT),data=SBWALTEM, method="REML") #using the function Gls because allows Predict function needed to plot trendline
AvNEMpredict<-Predict(SBWALTEMvaridentremlGLS, dummyPeak=c(1:14), Plot=seq(from=1,to=3,by=1),SBWALT=c("ALT","SBW"))%>%
  mutate(label=as.factor(ifelse(SBWALT=="SBW", "spruce budworm","other caterpillars")),Peak=dummyPeak-4,Plot=as.factor(Plot))

SBWALTEMpredict<-left_join(SBWALTEM,AvNEMpredict,by=c("Plot", "Peak", "SBWALT", "label"))
...
SBWALTEMplot<-SBWALTEMpredict%>%#PLACEHOLDER FIX LINES SO NOT FOR ALL YEARS BUT FOR EACH PLOT
  ...
  ylab("Number of parasitoid \nemergences")+xlab("Years before/after peak")+ylim(c(0,NA))+scale_shape_manual(values=c(16,17,15),guide=FALSE)+scale_linetype_manual(values=c("dotted","dashed","solid"))
...
numinteractionsspecies<-interaction%>%
  ...
  arrange(desc(numinter)) # Produces a dataframe of the total number of emergences with spruce budworm and other caterpillars for each parasitoid taxon. Then arranged descending from most emergences to lowest emergences.
...
bind_rows(drop_top3_sp("p01"), drop_top3_sp(c("p01","p02")),drop_top3_sp(c("p01","p02","p03")))#drop the most abundant parasitoid taxon (p01), then drop the two most abundant taxa, then drop the three most abundant taxa. compare coefficient values (average of all three plots), run one sample ttests and combine into one table.
...
stress10<-c(0.1344915,0.1194114,0.08352585,0.07981349,0.0798731,0.125509,0.07981351,0.08628342,0.08728616,0.08215742)#PLACEHOLODER CHANGE
sd(stress10)
...
bipartitegraphSBWALT<-interaction%>%
  ...
  frame2webs(varnames=c("SBWALT","SpecID.highnum","Peak")) # creates a dataframe that counts the number of interactions between each parasitoid taxon and spruce budworm or other caterpillars for each year.
...
allsharedprop<-interaction%>%
  ...
  mutate(PropSh=NSha/total)#90% of all parasitism emergences from spruce budworm were done by taxa that share spruce budworm and other caterpillars - 84% of all parasitism emergences from other caterpillars were done by shared parasitoids
